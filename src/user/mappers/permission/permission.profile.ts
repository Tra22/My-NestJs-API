...
export class PermissionProfile extends AutomapperProfile {
  ...
  override get profile() {
    return (mapper) => {
      ...
      //dto to model
      createMap(mapper, CreatePermissionDto, Permission, forMember((dest) => dest.id, ignore()));
      createMap(mapper, UpdatePermissionDto, Permission, forMember((dest) => dest.updatedAt, mapFrom(p => new Date())));
    };
  }
}